...
class RepoLayoutPresets {
    ...
    static void parallelDevelopmentOnTwoBranchesWithPullRequests(Repository repo) {
        ...
        */
        ...
        git.branch.add(name: 'fix/one', startPoint: 'main')
        ...
        def fixOnePr = repo.createPullRequest("A Fix PR 1", "fix/one", "main","")
        fixOnePr.setLabels("fix","one")
        repo.mergePullRequest(fixOnePr)

        git.checkout(branch: "main")
        git.pull(rebase:true, branch: "main")

        git.branch.add(name: 'develop', startPoint: 'main')
        ...
        git.branch.add(name: 'fix/two', startPoint: 'main')
        ...
        def fixTwoPr = repo.createPullRequest("A Fix PR 2", "fix/two", "main","")
        ...
        git.checkout(branch: "develop")
        git.pull(rebase:true, branch: "develop")
        git.merge(head: "main", mode: MergeOp.Mode.CREATE_COMMIT)
